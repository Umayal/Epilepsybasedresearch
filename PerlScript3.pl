		...
		for($i=0;$i<=$length;$i++){
				...
				if($chr[$i] eq $b[0]){
					...
					if(($b[1]>=$pos1[$i]) && ($b[1]<=$pos2[$i])) {
						...
						print O $chr[$i],"\t","HAVANA\tgene\t",$b[1],"\t",$pos2[$i],"\t.\t+\t.\tID=ENSG00000117394.24;gene_id=ENSG00000117394.24;gene_type=protein_coding;gene_name=SLC2A1;level=2;hgnc_id=HGNC:11005;havana_gene=OTTHUMG00000007657.12\n";
